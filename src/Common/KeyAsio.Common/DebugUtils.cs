...
public static class DebugUtils
{
        ...
        bool isLastItem, bool? includeFullType)
    {
        ...
        {
            if (ex is AggregateException { InnerException: { } innerException } agg)
            {
                var complexMessage = agg.Message;
                var i = complexMessage.IndexOf(innerException.Message, StringComparison.Ordinal);
                if (i == -1)
                    return complexMessage;
                return complexMessage[..Math.Max(0, i - 2)];
            }
